icasamp <-
  function(dname,query=c("rnd","pdf","kur"),nsamp=NULL,data=NULL){
    ...
    if(dname=="a"){
      ...
      prop=rep(0.5,2); mus=c(-1,1); covs=rep(0.5,2)
      if(query=="rnd"){
      ...
      prop=rep(0.5,2); mus=c(-0.5,0.5); covs=rep(.15,2)
      if(query=="rnd"){
        idx=sample(1:2,nsamp,replace=TRUE)
      ...
      prop=rep(0.5,2); mus=c(-0.5,0.5); covs=rep(0.4,2)
      if(query=="rnd"){
        idx=sample(1:2,nsamp,replace=TRUE)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
      ...
      prop=rep(0.5,2); mus=c(-0.5,0.5); covs=rep(0.5,2)
      if(query=="rnd"){
        idx=sample(1:2,nsamp,replace=TRUE)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
      ...
      prop=c(1,3); prop=prop/sum(prop); mus=c(-0.5,0.5); covs=rep(0.15,2)
      if(query=="rnd"){
        idx=sample(1:2,nsamp,replace=TRUE,prob=prop)
        return(rnorm(nsamp,mus[idx],covs[idx])-.25)
      } else if(query=="pdf"){
        myden=0
        for(i in 1:2){myden=myden+prop[i]*dnorm(data,mus[i],covs[i])}
        return(myden)
      ...
      prop=c(1,2); prop=prop/sum(prop); mus=c(-0.7,0.5); covs=rep(0.4,2)
      if(query=="rnd"){
        idx=sample(1:2,nsamp,replace=TRUE,prob=prop)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
      ...
      prop=c(1,2); prop=prop/sum(prop); mus=c(-0.7,0.5); covs=rep(0.5,2)
      if(query=="rnd"){
        idx=sample(1:2,nsamp,replace=TRUE,prob=prop)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
      ...
      prop=c(1,2,2,1); prop=prop/sum(prop); mus=c(-1,-.33,.33,1); covs=rep(0.16,4)
      if(query=="rnd"){
      ...
      prop=c(1,2,2,1); prop=prop/sum(prop); mus=c(-1,-.2,.2,1); covs=c(.2,.3,.3,.2)
      if(query=="rnd"){
        idx=sample(1:4,nsamp,replace=TRUE,prob=prop)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
      ...
      prop=c(1,2,2,1); prop=prop/sum(prop); mus=c(-.7,-.2,.2,.7); covs=c(.2,.3,.3,.2)
      if(query=="rnd"){
        idx=sample(1:4,nsamp,replace=TRUE,prob=prop)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
      ...
      prop=c(1,1,2,1); prop=prop/sum(prop); mus=c(-1,.3,-.3,1.1); covs=c(.2,.2,.2,.2)
      if(query=="rnd"){
        idx=sample(1:4,nsamp,replace=TRUE,prob=prop)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
      ...
      prop=c(1,3,2,.5); prop=prop/sum(prop); mus=c(-1,-.2,.3,1); covs=c(.2,.3,.2,.2)
      if(query=="rnd"){
        idx=sample(1:4,nsamp,replace=TRUE,prob=prop)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
      ...
      prop=c(1,2,2,1); prop=prop/sum(prop); mus=c(-.8,-.2,.2,.5); covs=c(.22,.3,.3,.2)
      if(query=="rnd"){
        idx=sample(1:4,nsamp,replace=TRUE,prob=prop)
        return(rnorm(nsamp,mus[idx],covs[idx]))
      } else if(query=="pdf"){
