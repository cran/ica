icafast <-
  function(X,nc,center=TRUE,maxit=100,tol=1e-6,Rmat=diag(nc),
    ...
    ### initial checks
    X=as.matrix(X);  nobs=nrow(X);  nvar=ncol(X)
    nc=as.integer(nc[1]); if(nc<1){stop("Must set nc>=1 component.")}
    maxit=as.integer(maxit[1]); if(maxit<1){stop("Must set maxit>=1 iteration.")}
    tol=tol[1]; if(tol<=0){stop("Must set ctol>0.")}
    ...
    ### check if nc=1
    if(nc==1L){return(list(S=Xw,M=Mprt,W=t(Pmat),Y=Xw,Q=t(Pmat),R=matrix(1),
    ...
    if(alg[1]=="def"){
      ...
      for(j in 1:nc){
        if (j<2){
          ...
          Rmat[,j]=Rmat[,j]/sqrt(sum(Rmat[,j]^2))
          svec=matrix(0,nc,1)
          for(k in 1:(j-1)){svec=svec+sum(Rmat[,k]*Rmat[,j])*Rmat[,k]}
          Rmat[,j]=Rmat[,j]-svec;  Rmat[,j]=Rmat[,j]/sqrt(sum(Rmat[,j]^2))
          # get j-th component
          iter=0;  vtol=1
          while(vtol>tol && iter<maxit){
            ...
            svec=matrix(0,nc,1)
            for(k in 1:(j-1)){svec=svec+sum(Rmat[,k]*rnew)*Rmat[,k]}
            rnew=rnew-svec;   rnew=rnew/sqrt(sum(rnew^2))
            # check for convergence
            vtol=1-abs(sum(Rmat[,j]*rnew))
            iter=iter+1;  Rmat[,j]=rnew
          }
    ...
    ### sort according to vafs
     ...
     M=M[ix,];  Rmat=Rmat[,ix];  vafs=(vafs[ix]*nobs)/sum(X^2)
